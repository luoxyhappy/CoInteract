...
class WanS2VAudioEncoder(torch.nn.Module):
    ...
    def extract_audio_feat(self, input_audio, sample_rate, processor, return_all_layers=False, dtype=torch.float32, device='cpu'):
        ...
        # 时间维度下采样,从16000hz下采样到50hz,维度是1024, feat的维度是[num_layers,total_frames,dim]
        res = self.model(input_values, output_hidden_states=True)
        if return_all_layers:
            feat = torch.cat(res.hidden_states)
        else:
            feat = res.hidden_states[-1]
        # 再从50hz插值回视频帧率对齐,例如25fps, feat的维度是[num_layers,total_frames,dim]
    ...
    def get_audio_embed_bucket_fps(self, audio_embed, fps=16, batch_frames=81, m=0, start_idx=0):
        num_layers, audio_frame_num, audio_dim = audio_embed.shape
        ...
        #audio_frame_num==750(30s音频转为25fps对应视频的帧数)
        min_batch_num = int(audio_frame_num / (batch_frames * scale)) + 1

        bucket_num = min_batch_num * batch_frames
        padd_audio_num = math.ceil(min_batch_num * batch_frames / fps * self.video_rate) - audio_frame_num
        batch_idx = get_sample_indices(
            original_fps=self.video_rate, total_frames=audio_frame_num + padd_audio_num, target_fps=fps, num_sample=bucket_num, fixed_start=start_idx
        )
        ...
        for bi in batch_idx:
            if bi < audio_frame_num:
                #实际上这部分也是构造滑动窗口,以bi为中心,前向取m个点,向后取m个点,步长是stride
                chosen_idx = list(range(bi - m * audio_sample_stride, bi + (m + 1) * audio_sample_stride, audio_sample_stride))
                chosen_idx = [0 if c < 0 else c for c in chosen_idx]
                chosen_idx = [audio_frame_num - 1 if c >= audio_frame_num else c for c in chosen_idx]

                if return_all_layers:
                    #num_layers * 2m+1(滑动窗口) * dim, flatten后把后两个维度拼接
                    frame_audio_embed = audio_embed[:, chosen_idx].flatten(start_dim=-2, end_dim=-1)
                else:
                    frame_audio_embed = audio_embed[0][chosen_idx].flatten()
            else:
            ...
            batch_audio_eb.append(frame_audio_embed)
        #audio_frame_num(中心点数量)*audio_layers*[(2m+1)*dim]
        batch_audio_eb = torch.cat([c.unsqueeze(0) for c in batch_audio_eb], dim=0)

        return batch_audio_eb, min_batch_num

    def get_audio_feats_per_inference(self, input_audio, sample_rate, processor, fps=16, batch_frames=80, m=0, dtype=torch.float32, device='cpu',start_idx=0):
        #print("params")
        #print(batch_frames)
        #print(fps)
        #print(m)
        ...
        audio_embed_bucket, min_batch_num = self.get_audio_embed_bucket_fps(audio_feat, fps=fps, batch_frames=batch_frames, m=m, start_idx=start_idx)
        audio_embed_bucket = audio_embed_bucket.unsqueeze(0).permute(0, 2, 3, 1).to(device, dtype) #[1,layer_num,(2m+1)*dim,T]
        audio_embeds = [audio_embed_bucket[..., i * batch_frames:(i + 1) * batch_frames] for i in range(min_batch_num)] #[1,layer_num,(2m+1)*dim,81]
        return audio_embeds
