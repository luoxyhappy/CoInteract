...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False, attention_mask: Optional[torch.Tensor] = None):
    """
    Flash Attention with optional attention mask support.
    
    Args:
        q: Query tensor, shape [B, S, D] where D = num_heads * head_dim
        k: Key tensor, shape [B, S, D]
        v: Value tensor, shape [B, S, D]
        num_heads: Number of attention heads
        compatibility_mode: Use PyTorch's scaled_dot_product_attention
        attention_mask: Optional attention mask, shape [1, 1, S, S] or [B, 1, S, S]
                       1.0 = attend, 0.0 = mask out (will be converted to -inf)
    
    Returns:
        Output tensor, shape [B, S, D]
    """
    # 如果有 attention_mask，需要使用支持 mask 的实现
    if attention_mask is not None:
        # 使用 PyTorch 的 scaled_dot_product_attention，它支持 attn_mask
        q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
        
        # 将 mask 从 [1, 1, S, S] 转换为 attention bias
        # attention_mask: 1.0 = attend, 0.0 = mask out
        # 需要转换为: 0.0 = attend, -inf = mask out
...
class AttentionModule(nn.Module):
    ...
    def forward(self, q, k, v, attention_mask: Optional[torch.Tensor] = None):
        """
        Attention forward with optional mask support.
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, attention_mask: Optional[torch.Tensor] = None):
        """
        Self-Attention forward with optional mask support.
        ...
        x = self.attn(q, k, v, attention_mask=attention_mask)
        return self.o(x)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, attention_mask: Optional[torch.Tensor] = None):
        """
        DiTBlock forward with optional attention mask support.
        ...
        Args:
            ...
            freqs: RoPE frequencies
            attention_mask: Optional attention mask for self-attention, shape [1, 1, S, S]
                           1.0 = attend, 0.0 = mask out
        """
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs, attention_mask=attention_mask))
